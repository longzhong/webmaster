...
;(function($){			
	...
	var optionDefaults = {
		...
		weatherStates : {
			//晴天 
			'sun'	: { length : 1 }, /* typesNum , child  */
			//多云
			'wind' 		: { length : 1 },
			//阴天
			'cloud sun' 	: { length : 2 },
			// 雨
			'hail': { length: 2, drop: [{ time: 300 }, { time: 300 }] },
			//雨夹雪
	...
	Weather.prototype = {
		...
		init : function(config){
			...
			var weatherInfo = this.opts.weatherData || {},
				...
				templeteHtml = '<div class="widget-details" style="height:53px"><small>{#day#} : {#day_weather#}</small> <span class="h3">{#day_temp#}{#temp#}</span><span class="h3"> - </span><small>{#night#} : {#night_weather#}</small> <span class="h3">{#night_temp#}{#temp#}</span><p class="text-uppercase small text-white"><span class="btn btn-success btn-xs btn-round">{#date#} ( {#city#} )</span></p></div><div class="widget-details"> <div class="climacon {#sky#} fa-3x"></div> </div>',
				i = 0
		    ;
			//alert(skyState_child_length);
