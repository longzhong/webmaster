(function () {
    ...
    D.fn = D.prototype = {
        ...
        filter: function (b) {
            return this.pushStack(D.isFunction(b) && D.grep(this,
            function (a, i) {
                return b.call(a, i)
    ...
    D.extend({
        ...
        className: {
            ...
            remove: function (c, b) {
                if (c.nodeType == 1) c.className = b != undefined ? D.grep(c.className.split(/\s+/),
                function (a) {
        ...
        grep: function (c, a, d) {
            var b = [];
            for (var i = 0,
            length = c.length; i < length; i++) if (!d != !a(c[i], i)) b.push(c[i]);
            return b
        },
        map: function (d, a) {
            var c = [];
    ...
    function (c, b) {
        D.fn[c] = function () {
            ...
            return this.each(function () {
                for (var i = 0,
                length = a.length; i < length; i++) D(a[i])[b](this)
            })
        }
    });
    D.each({
    ...
    D.event = {
        add: function (f, i, g, e) {
            ...
            handle = D.data(f, "handle") || D.data(f, "handle",
            function () {
    ...
    D.fn.extend({
        ...
        toggle: function (b) {
            ...
            return this.click(D.event.proxy(b,
            function (a) {
    ...
    function bindReady() {
        ...
        if (D.browser.opera) document.addEventListener("DOMContentLoaded",
        function () {
            if (D.isReady) return;
    ...
    D.each(("blur,focus,load,resize,scroll,unload,click,dblclick," + "mousedown,mouseup,mousemove,mouseover,mouseout,change,select," + "submit,keydown,keypress,keyup,error").split(","),
    function (i, b) {
        D.fn[b] = function (a) {
    ...
    D.fn.extend({
        ...
        serializeArray: function () {
            return this.map(function () {
                ...
                var b = D(this).val();
                return b == null ? null : b.constructor == Array ? D.map(b,
                function (a, i) {
                    return {
                        name: c.name,
                        value: a
    ...
    D.extend({
        ...
        lastModified: {},
        ajax: function (s) {
            s = D.extend(true, s, D.extend(true, {},
            D.ajaxSettings, s));
            var g, jsre = /=\?(&|$)/g,
        ...
        param: function (a) {
            var s = [];
            if (a.constructor == Array || a.jquery) D.each(a,
            function () {
                s.push(encodeURIComponent(this.name) + "=" + encodeURIComponent(this.value))
            });
            else for (var j in a) if (a[j] && a[j].constructor == Array) D.each(a[j],
            function () {
