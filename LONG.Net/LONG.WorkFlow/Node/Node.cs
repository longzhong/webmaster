...
namespace LONG.WorkFlow
{
	public class Node
	{
		...
		public void RemoveNode(Node delegateNode)
		{
			IList namedChildren = (IList) _children[delegateNode.Name];
			namedChildren.Remove(delegateNode);
			_content.Remove(delegateNode);
		}
		...
		public Node GetChildElement(String childName)
		{
			Node child = null;
			IList namedChildren = (IList) _children[childName];
			if (namedChildren != null)
		...
		public String GetProperty(String propertyName)
		{
			...
			{
				Node child = this.GetChildElement(propertyName);
		...
		public void GetContentString(StringBuilder buffer, String indentation)
		{
			...
			while (iter.MoveNext())
			{
				...
				if (contentItem is Node)
				{
					Node element = (Node) contentItem;
